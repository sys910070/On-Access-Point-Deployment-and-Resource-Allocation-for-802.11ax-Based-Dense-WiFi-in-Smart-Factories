...
def log_info(ap_list, device_list):
    ...
    for device in device_list:
        if device.ap != None:
            device_logger.info(f'{device.id}, {device.ap.id}, {device.x}, {device.y}, {device.throughput}')
        else:
            device_logger.info(f'{device.id}, {None}, {device.x}, {device.y}, {device.throughput}')
