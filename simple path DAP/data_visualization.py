...
def log_info(ap_list, device_list):
    ...
    for device in device_list:
        if device.ap != None:
            device_logger.info(f'{device.id}, {device.ap.id}, {device.power}, {device.x}, {device.y}, {device.timer}, {device.state.name}')
        else:
            device_logger.info(f'{device.id}, {None}, {device.power}, {device.x}, {device.timer}, {device.state.name}')
